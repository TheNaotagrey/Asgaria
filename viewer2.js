(() => {
  ...
  function applyViewModelData(vm, options = {}) {
    const { baronyMetaSource = null } = options;

    seigneurMap = { ...vm.seigneurs.byId };
    religionMap = { ...vm.religions.byId };
    cultureMapInfo = { ...vm.cultures.byId };
    viscountyMap = { ...vm.viscounties.byId };
    countyMap = { ...vm.counties.byId };
    marquisateMap = { ...vm.marquisates.byId };
    duchyMap = { ...vm.duchies.byId };
    archduchyMap = { ...vm.archduchies.byId };
    kingdomMap = { ...vm.kingdoms.byId };
    empireMap = { ...vm.empires.byId };
    baronyLookup = { ...vm.baronies.byId };
    baronyMeta = baronyMetaSource || { ...vm.baronies.byId };
    ...
    seigneurToKingdom = {};
    seigneurToEmpire = {};
    Object.entries(vm.indexes?.titlesBySeigneurId || {}).forEach(([seigneurId, titles]) => {
      seigneurToViscounty[seigneurId] = (titles?.viscounty || []).map((entry) => entry.id);
      seigneurToCounty[seigneurId] = (titles?.county || []).map((entry) => entry.id);
      seigneurToMarquisate[seigneurId] = (titles?.marquisate || []).map((entry) => entry.id);
      seigneurToDuchy[seigneurId] = (titles?.duchy || []).map((entry) => entry.id);
      seigneurToArchduchy[seigneurId] = (titles?.archduchy || []).map((entry) => entry.id);
      seigneurToKingdom[seigneurId] = (titles?.kingdom || []).map((entry) => entry.id);
      seigneurToEmpire[seigneurId] = (titles?.empire || []).map((entry) => entry.id);
    });
    ...
    vm.baronies.list.forEach((barony) => {
      if (Array.isArray(barony.canonicalFor) && barony.canonicalFor.length > 0) {
        canonicalLandMap[barony.id] = barony.canonicalFor.map((entry) => entry.id);
      }
      if (Array.isArray(barony.canonicalLands) && barony.canonicalLands.length > 0) {
        canonicalDependents[barony.id] = barony.canonicalLands.map((entry) => entry.id);
      }
      if (Array.isArray(barony.canonicalFor)) {
        const parentIds = barony.canonicalFor.map((entry) => entry.id).filter((id) => String(id) !== String(barony.id));
        if (parentIds.length > 0) {
          canonicalParents[barony.id] = parentIds;
        }
      }
    });
    ...
    sanctuaryMap = {};
    vm.baronies.list.forEach((barony) => {
      if (!Array.isArray(barony.sanctuaries) || barony.sanctuaries.length === 0) return;
      sanctuaryMap[barony.id] = barony.sanctuaries.map((entry) => ({ religion_id: entry.religion_id }));
    });
  ...
  async function fetchData() {
    ...
    if (mapMode === 'sea') {
      ...
      applyViewModelData(vm, { baronyMetaSource: toIndexedMap(zones) });
      baronyAdjacency = {};
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    baseMapLoaded.then(() => {
      ...
      core.ready.then(() => {
        filterManager = mapFilters2.init(core, mapData, { updateLegend });
