...
- **viewModel.js** : construit le modèle relationnel canonique (titres, liens de jure/de facto, terres canoniques, sanctuaires, connexions entre baronnies). Les UI doivent consommer ses références (`barony.dejure.*`, `barony.defacto.*`, `title.defactoParent`, `title.defactoChildren`, etc.) et ses helpers plutôt que reconstruire les hiérarchies localement.
...
- **viewer2.js**, **src/mapCore2.js**, **src/mapFilters2.js**, **src/viewModelLabels.js**, **src/mapInfoPanel2.js** : pile v2 compatible avec l’interface existante. `mapCore2` possède le runtime canvas partagé, expose le ViewModel, applique les filtres, maintient les cartes de couleur et sépare les clics carte (`onMapClick`) de la surbrillance (`highlightBaronies`). `viewer2` reprend le flux utilisateur de `viewer.js`, construit `viewModel.js` pendant le chargement des données et centralise la sélection métier via `selectEntity(...)`; il garde la logique propre à la page et prépare des données déjà dérivées du ViewModel. `mapInfoPanel2` rend les panneaux DOM sans reconstruire les hiérarchies. `mapFilters2` est un registre déclaratif de définitions de filtres (`colorForBarony`, `selectEntityForBaronyClick`, `legendEntityForBarony`, `kind`) sans runtime d’application ni alias legacy de sélection.
