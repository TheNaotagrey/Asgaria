...
- La base de données contient également une table `trade_transactions` (origine, destination, ressources, type, état, raison, décision, retour) pour enregistrer les échanges entre seigneuries. L'origine et la destination y sont stockées via les identifiants de seigneurie, les noms des seigneurs ou baronnies étant résolus dynamiquement. Les colonnes `origin_update_year`, `origin_update_number` et `received` permettent désormais de lier chaque envoi à la mise à jour de l'émetteur et de différer la réception effective jusqu'à la progression du destinataire. Les effets `land_transaction_max_per_month` et `naval_transaction_max_per_month` permettent d'augmenter les limites mensuelles de transactions.
- La table `seigneuries` inclut désormais `update_year` et `update_number` pour suivre la progression asynchrone des joueurs dans les 10 mises à jour annuelles (février à hiver). Les compteurs mensuels (`spells_cast`, transactions terrestres/navales) sont maintenant réinitialisés à chaque mise à jour joueur plutôt qu'au mois civil.
- `server.js` expose aussi `POST /api/seigneurie/advance_update`, qui valide côté serveur les blocages (par exemple surcharge de population employée), applique la production, la famine, les pertes par capacité maximale, réinitialise les compteurs de mise à jour et distribue les transactions commerciales approuvées devenues recevables.
