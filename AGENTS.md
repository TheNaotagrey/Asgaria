...
- La table `seigneuries` inclut `update_year` et `update_number` pour suivre la progression asynchrone des joueurs dans les 10 mises à jour annuelles (février à hiver). Les compteurs de période (`spells_cast`, `land_transactions`, `naval_transactions`) se réinitialisent à chaque mise à jour joueur.
